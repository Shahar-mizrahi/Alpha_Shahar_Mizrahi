...
for i, combination in enumerate(all_combinations):
    ...
    for j in range(NUM_OF_ITERATIONS):
        ...
        gradient_boosting = ee.Classifier.smileGradientTreeBoost(numberOfTrees= 154, shrinkage= 0.1, samplingRate= 1, maxNodes= 20, loss= 'LeastSquares', seed= 42).setOutputMode('REGRESSION')
...
csv_output_path = r"C:\Users\Cyber_User\Desktop\alpha_codes\code_results/GB_best_combination_results2222.csv"

# Add data to the CSV
for combination, nrmse_ in zip(all_combinations, nrmse_list):
    csv_data.append([", ".join(combination), str(round(nrmse_, 3)) + "%"])

# Write the data to the second CSV file
